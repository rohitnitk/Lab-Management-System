...
$result = $db->query('SELECT * FROM acids');
$array = array();
...
?>
		<div class="view"  >
			<div class="name" style="padding: 5px; ; height:auto;"><h4><?php echo $row["acid_name"];  ?><hr></h4>
			</div>
			
			<span class="a">
				<?php  echo '
				<img style="max-width: 100%; max-height: 100%;"src="data:image; base64,'.$row["acid_img"].' " >' ;?>
			</span>
			<span class="b" id="desc"  ><?php echo $row["acid_desc"]; ?>
			</span>
		
			<div class="status" >
				<div  style="background-color: #eeeeea	; width: auto; height: auto; padding: 5px; margin-bottom: 10px; overflow: auto;	">
					<label for="capacity">Capacity: <?php echo $row["acid_capacity"]; ?> </label>
					<label for="availabe">Availabel: <?php echo $row["acid_remaining"]; ?></label>

					<button  id = "<?php echo $i; ?>" onclick="updateF(<?php echo ($j); ?>)" >UPDATE</button>
					<div id="<?php echo($j); ?>" style="display:none" >
						<form   action="update_acid.php" style="width: 30%; " method="post" enctype="multipart/form-data">
							...
							<input type="hidden" name="capacity" value="<?php echo $array[$i]['acid_capacity']?>">
