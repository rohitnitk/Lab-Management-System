...
?>
		<div class="view"  >
			<div class="name" style="padding: 5px; ; height:auto;"><h4><?php echo $row["equip_name"]; ?><hr></h4>
			</div>
			
			<span class="a">
				<?php  echo '
				<img style="max-width: 100%; max-height: 100%;"src="data:image; base64,'.$row["equip_img"].' " >' ;?>
			</span>
			<span class="b" id="desc"  ><?php echo $row["equip_desc"]; ?>
			</span>
		
			<div class="status" >
				<div  style="background-color: #eeeeea	; width: auto; height: auto; padding: 5px; margin-bottom: 10px; overflow: auto;	">
					<label for="quantity">Capacity: <?php echo $row["equip_capacity"]; ?> </label>
					<label for="quantity">Quantity: <?php echo $row["equip_qt"]; ?> </label>
					

					<button  id = "<?php echo $i; ?>" onclick="updateF(<?php echo ($j); ?>)" >UPDATE</button>
					<div id="<?php echo($j); ?>" style="display:none" >
						<form   action="update_equip.php" style="width: 30%; " method="post" enctype="multipart/form-data">
